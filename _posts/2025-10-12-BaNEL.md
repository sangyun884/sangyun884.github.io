...
**Do not make the same mistake twice!** If our budget for evaluating $r$ were unlimited, and assuming the solution has bounded length, we could trivially achieve a perfect success rate by collecting every possible mistake $R:=\left\{\mathbf{x} \mid r(\mathbf{x})=0\right\}$ and avoiding all elements of $R$ :
...
$$
\max _{\boldsymbol{\phi}} \frac{1}{m} \sum_{i=1}^m \log p_{\boldsymbol{\phi}}\left(\mathbf{x}_i\right) .
$$
...
$$
\tilde{R}:=\left\{\mathbf{x}: \frac{p_{\boldsymbol{\theta}}(\mathbf{x})}{p_{\boldsymbol{\phi}}(\mathbf{x})}<\tau\right\}
$$
...
Here, rejection regions from earlier rounds can be accumulated by taking their union (i.e., $\tilde R \gets \tilde R \cup \tilde R_{\text{new}}$ where $R_{\text{new}}$ is the new rejection region).
...
![](/images/2025-10-12-BaNEL/algo.png)
